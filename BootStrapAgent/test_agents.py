"""
Simple Test Runner for Payment and Service Fulfillment Agents
Tests the agents with the new imports from agent.py
"""
...
# Avoid importing root_agent which requires vertexai
# Instead, we import specific classes directly from agent.py module
import importlib.util

# Mock the dependencies before importing agent.py
class MockModule:
...
sys.modules['vertexai'] = MockModule()
sys.modules['google'] = MockModule()
sys.modules['google.adk'] = MockModule()
sys.modules['google.adk.agents'] = MockModule()
sys.modules['google.adk.agents.callback_context'] = MockModule()
sys.modules['google.adk.models'] = MockModule()
sys.modules['google.adk.tools'] = MockModule()
sys.modules['google.genai'] = MockModule()
sys.modules['google.genai.types'] = MockModule()

# Now load agent.py directly without going through __init__.py
spec = importlib.util.spec_from_file_location(
    "agent_module", 
    bootstrap_path / "bootstrap_agent" / "agent.py"
)
...
AgentStatus = agent_module.AgentStatus
AgentMessage = agent_module.AgentMessage
Tool = agent_module.Tool
